...
const int WHITE_PIN = 4; // Scrolling -> OFF | Paging -> ON
...
// control flags
bool scrollMode = true; // 0 = false || 1 = true
  ...
  switch (action) {
    ...
    case START:
      if (action == prevAction) {
        ...
        if (scrollMode == true) { //Scroll mode
    ...
    case PREVIOUS:
      if (action == prevAction) {
        Serial.println((scrollMode == true) ? ("Scrolling UP") : ("Previous Page"));
        digitalWrite(RED_PIN, HIGH);
        digitalWrite(YELLOW_PIN, LOW);
        digitalWrite(GREEN_PIN, LOW);
        if (scrollMode) {
          Keyboard.write(0xDA); // UP ARROW to Scroll
        } else {
          Keyboard.write(0xD8); // Arrow left to Jump page
          delay(700);
        }
      }
      break;
    case NEXT:
      if (action == prevAction) {
        Serial.println((scrollMode == true) ? ("Scrolling DOWN") : ("Next Page"));
        digitalWrite(RED_PIN, LOW);
        digitalWrite(YELLOW_PIN, LOW);
        digitalWrite(GREEN_PIN, HIGH);
        if (scrollMode) {
          Keyboard.write(0xD9); // Down ARROW to Scroll
        } else {
          Keyboard.write(0xD7); // Arrow Right to Jump page
          delay(700);
        }
      }
      break;
